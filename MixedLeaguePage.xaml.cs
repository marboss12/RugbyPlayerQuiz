...
public partial class MixedLeaguePage : ContentPage, INotifyPropertyChanged
{
    private readonly MixedLeaguePlayerService _playerService;
    ...
    public MixedLeaguePage(MixedLeaguePlayerService playerService)
    {
        InitializeComponent();
    ...
    public MixedLeaguePage() : this(CreatePlayerService())
    {
    }

    // Фабричный метод для создания сервиса
    private static MixedLeaguePlayerService CreatePlayerService()
    {
        try
        {
            // Пробуем получить из DI контейнера
            var service = App.Current?.Handler?.MauiContext?.Services?.GetService<MixedLeaguePlayerService>();
            if (service != null)
            {
                return service;
            }
            ...
            return new MixedLeaguePlayerService(playerService);
        }
        catch (Exception ex)
        {
            // Аварийный вариант
            return new MixedLeaguePlayerService(new PlayerService());
        }
    }
